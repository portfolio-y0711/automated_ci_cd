...
for SERVER in $SERVER_LIST; do
  ...
  ssh -i /home/yoonsung0711/.ssh/id_rsa $USER_ID@$SERVER "(cd $DEPLOY_PATH/dist && rm config.json && mv config-deploy.json config.json)"
  ...
  ssh -i /home/yoonsung0711/.ssh/id_rsa $USER_ID@$SERVER ". ~/.nvm/nvm.sh && (cd $DEPLOY_PATH && nohup npm run start --prefix $DEPLOY_PATH > /dev/null 2>&1 &)"
  # ssh $USER_ID@$SERVER ". ~/.nvm/nvm.sh && (cd $DEPLOY_PATH && nohup npm run start --prefix $DEPLOY_PATH > /dev/null 2>&1 &)"
